...
import RNFS, { ExternalDirectoryPath,LibraryDirectoryPath } from 'react-native-fs';
...
class CacheImage extends React.Component {
    ...
    checkImageCache = (imageUri, cachePath, cacheKey) => {
        const dirPath = LibraryDirectoryPath+'/imageCaches' +'/'+cachePath;
    ...
    _processSource = (source, skipSourceCheck) => {
        ...
        { // remote 
            ...
            const cacheKey = md5.hex_md5(cacheable) + (type.length < url.pathname.length ? '.' + type : '');
